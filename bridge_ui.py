...
- Windows Autostart Configuration (New!)
"""
import os
import queue
import datetime
...
class BridgeUI:
    def __init__(
        ...
        on_toggle_autostart: Optional[callable] = None, # New Feature
        autostart_available: bool = False,              # New Feature
        autostart_enabled: bool = False,                # New Feature
    ...
    def _init_window(self):
        ...
        if CTK_AVAILABLE:
            ...
            self.root = ctk.CTk()
            self.root.geometry("850x660") # Slightly taller for settings
    ...
    def _build_ui_modern(self):
        ...
        # --- Progress Container (Stealth Mode) ---
        ...
        # 4. Control Deck & Settings
        ctrl_frame = ctk.CTkFrame(self.root, fg_color="transparent")
        ctrl_frame.pack(fill="x", padx=20, pady=5)

        # Left Side: File Watcher + Autostart
        left_ctrl = ctk.CTkFrame(ctrl_frame, fg_color="transparent")
        left_ctrl.pack(side="left")

        watch_label = ctk.CTkLabel(left_ctrl, text="Watching: ...", font=("Consolas", 10), text_color=t.text_dim)
        watch_label.pack(anchor="w")
        self.labels['watch'] = watch_label

        # --- Autostart Switch (Modern Style) ---
    ...
    def _toggle_autostart(self):
        """Callback when the autostart switch is clicked."""
        if not self.on_toggle_autostart or self.autostart_var is None:
            return
        
        # Get the new desired state from the variable
        ...
        # If it failed, revert the switch UI
        if not success:
            self.autostart_var.set(not desired)

    def set_autostart_enabled(self, enabled: bool):
        """Updates the switch UI externally."""
        self.autostart_enabled = enabled
        if self.autostart_var is not None and self.root is not None:
             try:
